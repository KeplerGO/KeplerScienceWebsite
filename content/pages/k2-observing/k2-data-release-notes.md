...
Campaign 9 was flown in the forward velocity vector direction to observe the star-rich region around the Galactic
bulge in search of microlensing events. In order to facilitate image reconstruction 
photometry the campaign was conducted in two parts with a ~0.6 pixel pixel offset between the two subfields
known as C9a and C9b. 
<br>
...
<li>  76 unique custom target were selected in C9. Three million of the total 3.3 million C9 target pixels were placed in 5 microlensing super apertures around the Galactic bulge. See the <a href="images/release-notes/c9/ktwoc091_caf.csv">csv file that maps</a> the custom aperture number to the target name.</li>
</ul>
...
<li>  132 late microlensing targets and 1 short-cadence target were added to the C9a target list for C9b. See the <a href="images/release-notes/c9/ktwoc092_caf.csv">csv file that maps</a> the custom aperture number to the target name.</li>
</ul>
...
<ul>
<li> The spacecraft observed in the forward velocity vector direction to allow for simultaneous observations of targets from from Earth.
...
<li> The campaign was split into two parts in order to allow for both the addition of late high-value microlensing targets and to allow for a subpixel pointing offset.
</ul>
...
During emergency mode the spacecraft clock was reset, so prior to the start of C9a data collection
an updated time was loaded to the clock. Subsequent timing observations on the ground determined that there
...
excursions during the campaign, one intended, one not.


<div class="thumbnail" style="width: 45%;display: inline-block;">
<div class="caption">
<i>Figure C9-Pointing History: pointing performance for C9 was consistent with that of previous campaigns. The roll amplitude and roll rate were in family with the campaigns using LGA1 and smaller than that seen in C7.</i>
</div>
...
<div class="caption">
<i>Figure C9-Roll Rate: the C9 roll rate as a function of solar is consistent with previous campaigns (C4 is shown here for comparison) despite the reversed motion of the Sun around the spacecraft due to the forward facing orientation.</i>
...
along the expected line of roll-motion (red x). The measured C9b position (green
...
A small uncommaned movement of the spacecraft of about -20 arc seconds in roll
...
resulting in the -20 arc second roll offset. The roll offset was corrected by
thrusters as part of the normal 6-hour K2 roll management cycle. 


<br>
***Calibrated Target Pixel Files***

This data release consits of calibrated target pixel files (TPFs) and supporting
calibration files. With
the use of large super apertures to collect the vast majority of the pixels and
because of the small number of standard targets, no light curve files were
generated for the C9 data. These calibrated TPFs are intended to replace the
raw uncalibrated cadence pixels files currently available at the MAST. These
raw files 
<ul>
...
We recommend that the calibrated TPFs delivered with Data Release 12 be used
...
pixel and light curve FITS files. Beacuse the C9 processing only used the pipeline
...
(C0, C1, C2). These TPFs have world coordinate system (WCS) coordinates 
based on the ra_dec_2_pix focal plane model and commanded pointing, 
rather than using a reconstructed poinitng history. We expect that the Type-1 WCS
...
export these data. There will be a new entry each time the data is released by
the mission.
