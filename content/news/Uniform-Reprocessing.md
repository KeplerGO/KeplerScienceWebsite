...
When there are more than one FFIs in a given campaign, the Kepler pipeline interpolates data between those FFIs, and extrapolates before and after each, to account for variations over time when calibrating the pixel-level data. A bug was discovered that if there were FFIs from campaigns later in time relative to the one currently being processed ingested in the database, those FFIs from later campaigns were used to interpolate for calibration of the current campaign. Since there are usually two FFIs per campaign, taken ~1/3 and ~2/3 the way through, this bug then would affect the pixel-level data (and thus lightcurves and other products as well) for the last ~1/3 of a campaign, with the latest cadences of the campaign affected most. The exact extent of the effect is not known, but is thought to be small given that the correction would be heavily weighted towards the correct campaign.

As shown in Figure Minor-Proc-Status, because campaigns were predominately processed in the order they were observed, this bug only affected C1 and C2, which were processed when FFIs for C3 were already ingested. The bug has been fixed in the pipeline code and so C1 and C2 will not suffer from it when reprocessed.
...
During the initial processing for a couple campaigns there were various reasons that some targets did not have target-pixel and/or light-curve data delivered. For C2, no lightcurves were delivered as the Kepler pipeline had not been fully adapted to work for K2 at the time, and until the code was finalized, it was deemed best to just produce target pixel files. For C2, C11a, C11b, and C14, there were 2–3 targets each that had an error when computing their centroids such that no WCS information was available and a FITS file was not able to be created for those targets, and thus no target pixel files nor lightcurves were delivered. For C11b, there were 191 targets that did not have delivered target pixel files or lightcurves due to a bug in how the pipeline FITS file creator was configured to run.

As shown in Figure Minor-Proc-Status, only C2, C11, and C14 had undelivered targets. For global reprocessing, all of the above issues have been fixed/addressed and thus the project expect to be able to successfully deliver the missing files when the affected campaigns are processed.
